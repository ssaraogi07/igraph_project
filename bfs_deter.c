...
igraph_bool_t igraph_bfs_callback(const igraph_t *graph,
...
int main() {
  ...
  bfs(&graph, /*root=*/0, /*roots=*/ 0, /*neimode=*/ IGRAPH_OUT, 
  ...
  bfs(&graph, /*root=*/ 0, /*roots=*/ 0, /*neimode=*/ IGRAPH_OUT, 
	     /*unreachable=*/ 1, /*restricted=*/ 0,
	     0, 0, 0, 0, 0, 0, &igraph_bfs_callback, 0);
  printf("\n");
  
  /* Test different roots */

  bfs(&graph, /*root=*/ 2, /*roots=*/ 0, /*neimode=*/ IGRAPH_OUT, 
	     /*unreachable=*/ 1, /*restricted=*/ 0,
	     0, 0, 0, 0, 0, 0, &igraph_bfs_callback, 0);
  printf("\n");
  ...
  bfs(&graph, /*root=*/ 5, /*roots=*/ 0, /*neimode=*/ IGRAPH_OUT, 
  ...
  bfs(&graph, /*root=*/ 4, /*roots=*/ 0, /*neimode=*/ IGRAPH_OUT, 
	     /*unreachable=*/ 1, &restricted,
	     0, 0, 0, 0, 0, 0, &igraph_bfs_callback, 0);
  printf("\n");  

  bfs(&graph, /*root=*/ 3, /*roots=*/ 0, /*neimode=*/ IGRAPH_OUT, 
  ...
  VECTOR(roots)[1]=4;
  VECTOR(roots)[2]=6;
  bfs(&graph, /*root=*/ -1, &roots, /*neimode=*/ IGRAPH_OUT, 
	     /*unreachable=*/ 0, &restricted,
	     0, 0, 0, 0, 0, 0, &igraph_bfs_callback, 0);
  printf("\n");    
...
int bfs(const igraph_t *graph, 
  ...
  if (!roots && (root < 0 || root >= no_of_nodes)) {
    IGRAPH_ERROR("Invalid root vertex in igraph_bfs", IGRAPH_EINVAL);
  }
...
# undef VINIT
	//SORT roots[]
  //igraph_vector_sort(igraph_vector_t *roots);
  //vector_print(roots);
  while (1) { 
    ...
    /* OK, we have a new root, start igraph_bfs */
