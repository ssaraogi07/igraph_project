...
int igraph_get_shortest_dij(const igraph_t *graph,
  ...
  while (!igraph_2wheap_empty(&Q) && to_reach > 0) {
    ...
    //printf("The value of minnei is : %li ",minnei);
    igraph_real_t mindist=-igraph_2wheap_delete_max(&Q);
    //printf("The value of mindist is : %li ",(long int)mindist); 
    ...
    for (i=0; i<nlen; i++) {
      ...
      //printf("The value of edge is : %li and the value of tto is %li",edge,tto);
      //printf("Printing neis... ");
      //igraph_vector_print(neis);
      igraph_real_t altdist=mindist + VECTOR(*weights)[edge];
      igraph_real_t curdist=VECTOR(dists)[tto];
      //printf("The value of curdist is %li",(long int)curdist);
      if (curdist < 0) {
	...
	long int number = rand()%100 +1;
	printf("The random number generated is %li",number);
	if(number>50)
	{
		parents[tto] = edge + 1;
	}
...
int main() {
  ...
  igraph_get_shortest_dij(&g, /*vertices=*/ &vecs, 
